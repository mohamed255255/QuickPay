...
public interface FavouriteRepository extends JpaRepository<favourites, Long> {
  ...
  @Query(
      value = "DELETE FROM favourites WHERE servicename = :servicename AND company = :company",
      nativeQuery = true)
  void deleteByServicenameAndCompany(
      @Param("servicename") String servicename, @Param("company") String company);
}
