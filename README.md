 ...
 2. A dashboard page that conatins the following :
     ...
     - Favourite button that shows all the favorite services the user uses most of the time (the user chooses by himself)
     - New payment button that transfer the user to the service category page

 3. payment page : 
     - After choosing the desired service the user is transferred to this page , at first the user has to pick a credit card and then 
     add the balance amount , the system will consider it as a payment transaction and shows a recipt for it.
    
 4. A heart button to mark the service as favorite and also the service can be removed if the user disable the heart button 
